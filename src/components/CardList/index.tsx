...
const CardList: React.FC<cardListType> = (props) => {
  ...
  React.useEffect(() => {
    if (cardListItems.length === 0) setIsLoading(true);
  }, [cardListItems]);

  const renderEmptyComponent = React.useCallback(
    () =>
      isLoading || cardListItems.length ? (
  ...
  return cardListItems.length ? renderVariant() : renderEmptyComponent();
};
